...
function aioseop_mrt_i() {
  ...
  add_options_page('All in One SEO Pack Importer', 'AIOSEOP Importer', 'manage_options', 'aioseopi', 'aioseopi_mrt_options');
...
<div>
<?php
	if($_POST['action'] == 'import_thesis' && $nonce = $_POST['nonce-aioseopi-migrate']){
		if (!wp_verify_nonce($nonce, 'aioseopi-migrate-nonce')) die ( 'Security Check - If you receive this in error, log out and back in to WordPress');
		else {
...
<div style="float:left;background-color:white;padding: 10px 10px 10px 10px;margin-right:15px;border: 1px solid #ddd;">
	...
	global $wpdb;
	$aioseoptitlecount = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $wpdb->postmeta WHERE meta_key='_aioseop_title';"));
	$aioseopdesccount = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $wpdb->postmeta WHERE meta_key='_aioseop_description';"));
	$aioseopkeywordscount = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $wpdb->postmeta WHERE meta_key='_aioseop_keywords';"));
...
<div style="float:left;background-color:white;padding: 10px 10px 10px 10px;margin-right:15px;border: 1px solid #ddd;">
	...
	$thesistitlecount = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $wpdb->postmeta WHERE meta_key='thesis_title';"));
	$thesisdesccount = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $wpdb->postmeta WHERE meta_key='thesis_description';"));
	$thesiskeywordscount = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $wpdb->postmeta WHERE meta_key='thesis_keywords';"));
...
<input type='hidden' name='nonce-aioseopi-migrate' value="<?php echo wp_create_nonce('aioseopi-migrate-nonce'); ?>" />
<input type="hidden" name="action" value="import_thesis" />

<p class="submit">
<input type="submit" class="button-primary" value="<?php _e('Import Thesis SEO Data') ?>" />
</p>
