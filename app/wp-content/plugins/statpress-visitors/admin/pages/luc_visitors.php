...
function luc_visitors()
      {     global $wpdb;
			...
			$strqry="SELECT  count(distinct post_name)
		                     ...
		                     WHERE p.post_status = 'publish' AND t.feed='' AND (p.post_type = 'page' OR p.post_type = 'post') AND t.date BETWEEN $limitdate AND $currentdate 
							 ";				   
			...
			echo "<div class='wrap'>
			           ...
			           <thead><tr>
				       <th scope='col' width='15%'><div style='background:$visitors_color;width:10px;height:10px;float:left;margin-top:2px;margin-right:5px;'></div>".$total->totalvisitors." visitors
			           <th scope='col' width='15%'><div style='background:$visitors_color;width:10px;height:10px;float:left;margin-top:2px;margin-right:5px;'></div>Average ".round($total->totalvisitors/$graphdays,1)." by day";
			if ($url->urlrequested == '') $out_url = "Page : Home";
			else  $out_url=$permalink[0].$url->post_name;
			echo "<th scope='col'><font font-size='1'>".$out_url  ."</th></font></tr></thead></table><table width='100%' border='0'><tr>";
